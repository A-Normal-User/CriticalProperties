...
# code is from https://github.com/marco-hoffmann/GRAPPA
...
class GAT_ResNet(nn.Module):
    """Implementation of GAT ResNet"""
    def __init__(self, node_dim, edge_dim, heads=5, dropout=0.1, concat=False, share_weights=True, num_layers = 2):
        ...
        for _ in range(num_layers):
            self.convs.append(GATv2Conv(node_dim, node_dim, heads, edge_dim=edge_dim, dropout=dropout, concat=False))
...
class GNN_GAT_Res(nn.Module):
    """Implementation of GAT"""

    def __init__(self, node_dim, edge_dim, conv_dim, heads=5, dropout=0.1, block_size = 2, num_layers = 2):
        """Initializes GAT model. Takes in node and edge dimensions conv_dim is the hidden dimension
        of the GAT convolutional layers. Heads is the number of attention heads to use in the GAT"""
        super().__init__()
        ...
        for _ in range(block_size):
            self.convs.append(GAT_ResNet(conv_dim, edge_dim=edge_dim, heads=heads, dropout=dropout, concat=False, share_weights=True, num_layers=num_layers))
...
class Head_ResNet(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_hidden_layers, out_dim):
        ...
        for _ in range(num_hidden_layers):
            #layers.append(nn.BatchNorm1d(hidden_dim))
            layers.append(Residual_2_2_gelu(hidden_dim))
...
class GNN_ResNet(nn.Module):
    def __init__(self, node_dim, edge_dim,
                 conv_dim, block_size, hidden_dim, num_hidden_layers, 
                 dropout,
                 num_gnn_layers=3, gnn_heads = 5, pooling_heads = 1):
        super().__init__()

        # GNN that graphs are passed to
        self.gnn = GNN_GAT_Res(node_dim, edge_dim, conv_dim, heads=gnn_heads, dropout=dropout, block_size=2, num_layers=num_gnn_layers)
        ...
        self.head = Head_ResNet(self.out_dim +2, hidden_dim, num_hidden_layers, 1)
